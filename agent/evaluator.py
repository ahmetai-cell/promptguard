...
def _eval_python_fallback(text: str) -> dict:
    ...
    PATTERNS = [
        ...
        # P189: 3rd-person "no restrictions/policy/training" persona
        (r"(?:an?\s+)?(?:ai|assistant|model|bot|chatbot|llm)\s+(?:called|named|\w+\s+)?(?:who|that|with)\s+(?:has\s+)?no\s+(?:content\s+)?(?:restrictions?|policy|guidelines?|training|oversight|censorship|filter)", 0.85),
        ...
        # P192: Simulation/sandbox framing + unrestricted AI
        (r"(?:simulation|virtual\s+(?:environment|world|scenario)|sandbox|sandboxed)\b.{0,150}(?:no\s+(?:content\s+)?(?:restrictions?|policy|guidelines?|filter|oversight)|unrestricted|without\s+(?:safety|restrictions?|guidelines?))", 0.85),
        # P193: Academic/research framing to extract unsafe AI behavior
        (r"(?:academic|research|paper|study|experiment|demo(?:nstrate)?)\b.{0,120}(?:no\s+safety|without\s+(?:safety|restrictions?|guidelines?|training|oversight)|as\s+if\s+(?:it\s+had|you\s+had)\s+no|had\s+no\s+(?:safety|restrictions?|training))", 0.85),
    ]
